...
/// </summary>
internal abstract class GenerateSettings : CommandSettings, IConnectionConfiguration
{
    /// <summary>
    /// Optional.
    ...
    /// Defaults to 'products' or 'employees' depending on the command.
    /// </summary>
    [Description("Optional. The name of the container to use. Defaults to 'products' or 'employees' depending on the command.")]
    [CommandOption("-c|--container-name <CONTAINER_NAME>")]
    public virtual string ContainerName { get; init; } = "items";
    ...
    public override ValidationResult Validate()
    {
        return (Endpoint, ConnectionString, Emulator) switch
        {
            ...
            (not null, _, true) => ValidationResult.Error("You can't provide an endpoint when using the emulator."),
            (not null, not null, _) => ValidationResult.Error("You can't provide both an endpoint and a connection string."),
            ("", _, _) => ValidationResult.Error("You must provide a valid value for the endpoint."),
